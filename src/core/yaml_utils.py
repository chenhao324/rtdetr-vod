...
def load_config(file_path, cfg=dict()):
    ...
    with open(file_path) as f:
        file_cfg = yaml.load(f, Loader=yaml.Loader)   # dict {'__include__': ['../dataset/coco_detection.yml', '../runtime.yml', './include/dataloader.yml', './include/optimizer.yml', './include/rtdetr_r50vd.yml'], 'output_dir': './output/rtdetr_r18vd_6x_coco', 'PResNet': {'depth': 18, 'freeze_at': -1, 'freeze_norm': False, 'pretrained': True}, 'HybridEncoder': {'in_channels': [128, 256, 512], 'hidden_dim': 256, 'expansion': 0.5}, 'RTDETRTransformer': {'eval_idx': -1, 'num_decoder_layers': 3, 'num_denoising': 100}, 'optimizer': {'type': 'AdamW', 'params': [{'params': '^(?=.*backbone)(?=.*norm).*$', 'lr': 1e-05, 'weight_decay': 0.0}, {'params': '^(?=.*backbone)(?!.*norm).*$', 'lr': 1e-05}, {'params': '^(?=.*(?:encoder|decoder))(?=.*(?:norm|bias)).*$', 'weight_decay': 0.0}], 'lr': 0.0001, 'betas': [0.9, 0.999], 'weight_decay': 0.0001}}
        if file_cfg is None:
            return {}

    if INCLUDE_KEY in file_cfg:
        base_yamls = list(file_cfg[INCLUDE_KEY])   # list ['../dataset/coco_detection.yml', '../runtime.yml', './include/dataloader.yml', './include/optimizer.yml', './include/rtdetr_r50vd.yml']
        for base_yaml in base_yamls:
            if base_yaml.startswith('~'):
                base_yaml = os.path.expanduser(base_yaml)

            if not base_yaml.startswith('/'):
                base_yaml = os.path.join(os.path.dirname(file_path), base_yaml)      # 'configs/rtdetr/../dataset/coco_detection.yml'
