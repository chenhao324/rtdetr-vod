...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, num_token=4000, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.flatten:
            x = x.reshape(1, -1, 256)  # BCHW -> BNC  (b, 196, 192)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if not self.fused_add_norm:
            ...
            if residual is None:
                hidden_states, residual = fused_add_norm_fn(  # (b, 197, 192),  (b, 197, 192)
                    hidden_states,
                    self.norm.weight,
                    self.norm.bias,
                    residual=residual,
        ...
        hidden_states = self.mixer(hidden_states, inference_params=inference_params)  # (b, 197, 192)
...
class VisionMamba(nn.Module):
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(patch_size=patch_size, stride=stride, in_chans=channels, embed_dim=embed_dim)
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_cls_token:
            if self.use_double_cls_token:
                ...
                if self.use_middle_cls_token:
                    cls_token = self.cls_token.expand(B, -1, -1)  # (b, 1, 192)
                    token_position = M // 2  # 98
                    # add cls token in the middle
                    x = torch.cat((x[:, :token_position, :], cls_token, x[:, token_position:, :]), dim=1)  # (b, 197, 192)
        ...
        if not self.if_bidirectional:
            for layer in self.layers:
                ...
                hidden_states, residual = layer(  # (b, 197, 192),  (b, 197, 192)
        ...
        if not self.fused_add_norm:
            ...
            hidden_states = fused_add_norm_fn(  # (b, 197, 192)
                self.drop_path(hidden_states),
                self.norm_f.weight,
                self.norm_f.bias,
                eps=self.norm_f.eps,
    ...
    # x(b, 3, 224, 224)
    def forward(self, input, return_features=True, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        x = self.forward_features(input, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)# (1, 2*500, 256)
...
if __name__ == '__main__':
    #data = torch.rand([2, 3, 224, 224]).to(torch.device('cuda'))
    data = torch.rand([2, 500, 256]).to(torch.device('cuda'))
    # model = create_model(
    ...
    #     img_size=224)
    model = VisionMamba(patch_size=16, embed_dim=256, depth=1, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='all',
